...
# 5. mAP 评估（需手动准备 PubLayNet val 数据，见"评估状态"）
...
- **待数据到位后**：将 `val.json` 放到 `data/raw/publaynet_val.json`，图片放到 `data/raw/publaynet_val_images/`，安装 `pycocotools`，运行 `python scripts/evaluate.py --quick`，得到 mAP 后回填本 README 与 `reports/metrics.json`。
...
│   ├── evaluate.py           # pycocotools mAP（占位：待数据 + pycocotools）
