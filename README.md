...
# 4. 单图推理
python scripts/detect.py --image samples/sample_paper.png
```
...
> 样本文档由 `scripts/make_samples.py` 用系统字体渲染生成（含真实文字，使 OCR 能识别 text 区域）。

## 评估状态

mAP 量化评估需要 PubLayNet val 数据（11K 图，COCO format）。该数据集的获取当前受网络限制（IBM DAX 链接不可靠，HF 镜像为 parquet 格式且连接不稳定）。

- **`evaluate.py` 代码已就绪**：pycocotools COCOeval，mAP@[0.50:0.95] + mAP@0.50 + 每类 AP
- **待数据到位后**：将 `val.json` 放到 `data/raw/`，运行 `python scripts/evaluate.py --quick` 即可产出指标
...
│   ├── evaluate.py           # pycocotools mAP（待 PubLayNet 数据）
