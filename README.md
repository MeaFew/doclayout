...
# 5. mAP 评估（需手动准备 PubLayNet val 数据，见下方）
...
- **待数据到位后**：将 `val.json` 放到 `data/raw/publaynet_val.json`，图片放到 `data/raw/publaynet_val_images/`，安装 `pycocotools`，再运行 `python scripts/evaluate.py --quick`。
