...
> Sample documents are generated by `scripts/make_samples.py` using system fonts (real text so OCR can recognize `text` regions).
...
- **To run evaluation**: place `val.json` at `data/raw/publaynet_val.json`, images in `data/raw/publaynet_val_images/`, install `pycocotools`, then run `python scripts/evaluate.py --quick`. Once mAP is computed, it will be back-filled into this README and `reports/metrics.json`.
...
│   ├── evaluate.py           # pycocotools mAP (placeholder: pending data + pycocotools)
